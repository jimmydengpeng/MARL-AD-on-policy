...
def init_agent(args: Arguments, gpu_id: int, env=None) -> AgentBase:
    debug_msg("initializing agent...", level=LogLevel.INFO)
    ...
    if env is not None:
        ...
        if args.env_num == 1:
            state = env.reset() # -> Tuple[obs: array, info: dict]
            if isinstance(state, Tuple):
                debug_msg("New version Gym, reset() -> tuple")
                state = state[0]
                print(state)
...
def train_and_evaluate(args):
    ...
    debug_print("agent.criterion", args=agent.criterion)
