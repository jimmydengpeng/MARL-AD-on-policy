...
def get_gym_env_args(env, if_print) -> dict:  # [ElegantRL.2021.12.12]
    ...
    if {'unwrapped', 'observation_space', 'action_space', 'spec'}.issubset(dir(env)):  # isinstance(env, gym.Env):
        ...
        if_discrete = isinstance(env.action_space, gym.spaces.discrete.Discrete)
        if if_discrete:  # make sure it is discrete action space
            action_dim = env.action_space.n
        elif isinstance(env.action_space, gym.spaces.box.Box):  # make sure it is continuous action space
...
def build_env(env=None, env_func: Optional[Callable] = None, env_args: Optional[dict] = None):  # [ElegantRL.2021.12.12]
    debug_msg("builing env...", level=LogLevel.INFO)
    if env is not None:
    ...
    debug_print("env built:", level=LogLevel.INFO, args=env)
    return env
